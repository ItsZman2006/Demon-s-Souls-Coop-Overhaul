...
--	クマムシ用戦闘
--	説明：
--　迷路で通せんぼ役なので、飛び跳ねたりしない
--　（ステップ(700)や攻撃(3002)を使用しない）
--
--	パラメータ0  なし
--	パラメータ1  なし
--	パラメータ2  なし
--	パラメータ3　なし
-----------------------------------------------------------------------------------------------
REGISTER_GOAL(GOAL_kuma313002_Battle, "kuma313002_Battle");


-- Updateでなにもやっていないので、呼ばないようにする
REGISTER_GOAL_NO_UPDATE( GOAL_kuma313002_Battle, 1 );


---------------------------------------------------------
--  ゴール起動
---------------------------------------------------------
function kuma313002_Battle_Activate(ai, goal)
-------------------------------------------------------------------------------
...
local role = ai:GetTeamOrder(ORDER_TYPE_Role);					--自分の役割を取得
		...
		elseif targetDist >= maaiDist then
			...
			--待機する
			if fate < 60 then
				goal:AddSubGoal( GOAL_COMMON_ApproachTarget, 10.0, TARGET_ENE_0, (maaiDist - 0.3), TARGET_ENE_0, true, -1 );
			else
--~ 				goal:AddSubGoal(GOAL_COMMON_Step, 5, 700, 0, 0, 0);
		...
		--間合い以内なら
			...
			--攻撃する
			kuma313002_Battle_Kougeki(ai, goal)
			
			--攻撃後行動
			kuma313002_Battle_Kougeki_Noato(ai, goal)
...
--  攻撃
---------------------------------------------------------
function kuma313002_Battle_Kougeki(ai, goal)
...
--  攻撃後の行動
---------------------------------------------------------
function kuma313002_Battle_Kougeki_Noato(ai, goal)
...
--  ゴール更新
---------------------------------------------------------
function kuma313002_Battle_Update(ai, goal)
...
--  ゴール終了
---------------------------------------------------------
function kuma313002_Battle_Terminate(ai, goal)
end
...
---------------------------------------------------------
function kuma313002_Battle_Interupt(ai, goal)
