...
--	パラメータ3　なし
-----------------------------------------------------------------------------------------------
REGISTER_GOAL(GOAL_IppanLONG_B201001_Battle, "ippanLONG_B201001Battle");


-- Updateでなにもやっていないので、呼ばないようにする
REGISTER_GOAL_NO_UPDATE( GOAL_IppanLONG_B201001_Battle, 1 );


---------------------------------------------------------
--  ゴール起動
---------------------------------------------------------
function ippanLONG_B201001Battle_Activate(ai, goal)
-------------------------------------------------------------------------------
...
local fateG = ai:GetRandam_Int(1,100)					--確率判定で使用する運命の数字　番外編
	...
	--ちょっと遠くて、自分の方が上にいるなら
	elseif targetDist >= 5.0 and targetDistY < -1.0 then
		Act01Per = 100		--何もしない
		Act02Per = 0		--その場で投げる
		Act03Per = 0		--（予備）
		Act04Per = 0		--（予備）	
	...
	elseif targetDist >= 5.0 then
		...
		Act03Per = 0		--（予備）
		Act04Per = 0		--（予備）	
		Act05Per = 90		--歩行間合い調整＆火炎瓶
		Act06Per = 10		--歩行接近＆近接攻撃
		Act07Per = 0		--蹴る
		Act08Per = 0		--回復を使う
	...
	--もっと近いなら
		...
		Act03Per = 0		--（予備）
		Act04Per = 0		--（予備）	
		Act05Per = 20		--歩行間合い調整＆火炎瓶
		Act06Per = 40		--歩行接近＆近接攻撃
		Act07Per = 40		--蹴る
		Act08Per = 0		--回復を使う
	...
	if role == ROLE_TYPE_Attack or role == ROLE_TYPE_None then
		...
		elseif fate2 <= (Act01Per + Act02Per + Act03Per + Act04Per + Act05Per + Act06Per) then	
			...
			ippanLONG_B201001Battle_Kougeki(ai, goal)	
			ippanLONG_B201001Battle_Kougeki_Noato(ai, goal, Guard)	
...
--  攻撃
---------------------------------------------------------
function ippanLONG_B201001Battle_Kougeki(ai, goal)
...
--  攻撃後の行動
---------------------------------------------------------
function ippanLONG_B201001Battle_Kougeki_Noato(ai, goal, Guard)
...
--  ゴール更新
---------------------------------------------------------
function ippanLONG_B201001Battle_Update(ai, goal)
...
--  ゴール終了
---------------------------------------------------------
function ippanLONG_B201001Battle_Terminate(ai, goal)
	--非戦闘時のアニメセットに切り替えます
...
---------------------------------------------------------
function ippanLONG_B201001Battle_Interupt(ai, goal)
