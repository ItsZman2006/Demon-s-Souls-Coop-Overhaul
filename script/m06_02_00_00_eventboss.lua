...
--DRAGON_EVENTDAMAGE = 100;--ダメージリアクションするHP
DRAGON_EVENTDAMAGE = 64.52;--ダメージリアクションするHP
DRAGON_DAMAGE_RATE = 1.0;--ダメージ控除率
DRAGON_NODAMAGE = 10000000;--無敵箇所のHP
...
function EventBoss_Initialize_m06_02_00_00(proxy)
	...
	--[[
	SingleReset( proxy , 1130 );--1回
	SingleReset( proxy , 1131 );--2回
	SingleReset( proxy , 1132 );--3回
	SingleReset( proxy , 1133 );--4回(破壊)	
	SingleReset( proxy , 1134 );--破壊	
	--1回も当ててない
...
function BossInit_m06_02_00_00_Second_Initialize(proxy,param)
	...
	--proxy:DisableDamage( 825, true);--ダメージ入らない
	proxy:SetSuperArmor( 825, true);--スーパーアーマー
	if proxy:CreateEventBody( 1104,825,"OnEvent_1104",DRAGON_EVENTDAMAGE,DRAGON_DAMAGE_RATE,everytime) == true then		
		...
		proxy:SetDisableDamage( 1104 , true );--部位ダメージ無効
...
function BossInit_DragonDemon(proxy,param)	
	...
	--■1070,1071,1072■ボス待機　【縫いつけ前】■
		if proxy:IsCompleteEvent( 1070 ) == false then
			proxy:Warp( 825, 2460);--位置あわせ
			if proxy:IsGreyGhost() == true or proxy:IsLivePlayer() == true then
				...
				proxy:LuaCallStartPlus( 1090, 2, 410 );--待機立ち見て無いを同期
			else
				--取り合えず初回のアニメ同期は保留
			end			
		end
...
function ChangeWait_2(proxy,param)
	...
	--proxy:LuaCallStartPlus( 1101, 3, 1 );--1091フラグON同期
	proxy:SetEventCommand( 825, 440);
	--proxy:DisableDamage( 825, false);--ダメージ入る
	proxy:SetDisableDamage( 1104 , false );--部位ダメージ有効
	proxy:Warp( 825, 2462);--位置あわせ
end
...
function OnEvent_1070(proxy,param)
	...
	proxy:LuaCallStartPlus( 1101, 1, 0);--1072OFF同期
...
function OnEvent_1071(proxy,param)
	...
	proxy:LuaCallStartPlus( 1101, 1, 1 );--1072ON同期
...
function OnEvent_1080(proxy,param)
	...
	proxy:LuaCallStartPlus( 1101, 2, 1 );--1082ON同期
...
function OnEvent_1081(proxy,param)
	...
	proxy:LuaCallStartPlus( 1101, 2, 0 );--1082OFF同期
...
NOW_OBJSTATE1	= OBJ_STATE_0;	--現在の状態
NOW_OBJSTATE2	= OBJ_STATE_0;	--現在の状態
NOW_OBJSTATE3	= OBJ_STATE_0;	--現在の状態
NOW_OBJSTATE4	= OBJ_STATE_0;	--現在の状態
NOW_OBJSTATE5	= OBJ_STATE_0;	--現在の状態
NOW_OBJSTATE6	= OBJ_STATE_0;	--現在の状態
NOW_OBJSTATE7	= OBJ_STATE_0;	--現在の状態
NOW_OBJSTATE8	= OBJ_STATE_0;	--現在の状態
NOW_OBJSTATE9	= OBJ_STATE_0;	--現在の状態
NOW_OBJSTATE10	= OBJ_STATE_0;	--現在の状態
...
function ObjDam(proxy,param,eventId,objId,nowState)
	...
	if nowState < State then
		...
		--変わったかもしれないので現HPと状態を配信? param2=objId,param3=State;
	...
	if nowState < State then
		...
		if State ~= OBJ_BREAK then			
			...
			if proxy:IsDestroyed( objId ) == false then
				proxy:PlayAnimation( objId, State );--OBJ個別のアニメ				
			end
		else
			print("Break");
			--proxy:ChangeModel( objId , 1 );--OBJ個別の破壊	
			--SetFirstSpeedPlus( objId , 2, 3.5  ,6 , 1);
			--proxy:SetBrokenPiece( objId );
			proxy:PlayTypeSE( objId, SOUND_TYPE_O, soundId );
			proxy:DeleteEvent(eventId);--イベントの破棄
