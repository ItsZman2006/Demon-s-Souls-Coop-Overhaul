...
KimeraCallPartner_HpRate	= 0.15;--相棒を呼ぶHP条件。1で100%  0.5で50%
KimeraCallPartner_Timer		= 100.0;--相棒を呼ぶまでの秒数
...
function EventBoss_Initialize_m04_01_00_00(proxy)
	...
	if proxy:IsCompleteEvent( 5251 ) == false then
		...
		--しっぽ用のキメラが余計なことをしないようにロジックOFF、完全無敵を入れておく
...
--ボスイベントアクションID領域(5308～5357)
--ボス2体の807,817イベントID
--ボス死亡のアクションID5251
--EzState EzState→Event　悲鳴開始(300), 割り込みで悲鳴終了(301)
--		  Event→EzState　悲鳴終了させる(500)
...
------------------------------------------------------------------------------------------------------------
--悲鳴状態によってロジックに値を投げて、相方に悲鳴を上げさせないようにする
...
------------------------------------------------------------------------------------------------------------
--秒数経過によりEventからEzStateに悲鳴終了を投げる
...
function OnEvent_5310(proxy,param)
	...
	proxy:PlayLoopAnimation( sub_evid, 9710 );--通常の死亡待機だとボスなのでアルファで消えてしまうため、特殊死亡待機へ
	proxy:EnableHide( sub_evid,true );--隠れます
