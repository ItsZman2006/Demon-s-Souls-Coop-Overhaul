...
function OnEvent_2000(proxy,param)
	...
	if nowAI_event == AI_EVENT_None then--í èÌÇ»ÇÁì{ÇÈ
		proxy:SendEventRequest(825,0,AI_EVENT_FlameMan_Angry);
	elseif nowAI_event == AI_EVENT_FlameMan_Angry then--ì{Ç¡ÇƒÇÈÇ»ÇÁé„ÇÈ
		proxy:SendEventRequest(825,0,AI_EVENT_FlameMan_Weaker);
		proxy:SendEventRequest(825,1,AI_EVENT_None);
	elseif nowAI_event == AI_EVENT_FlameMan_Weaker then--é„Ç¡ÇƒÇÈÇ»ÇÁí èÌÇ…
...
function OnEvent_2005(proxy,param)
	...
	proxy:SendEventRequest(825,1,AI_EVENT_FlameMan_Roar);--AIÇ…Ç‡ôÙöKÇ™èIÇÌÇ¡ÇΩéñÇí ím
